...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($company_name)) {
        $errors['register_errors']['company_name'] = 'Company name is required';
    }

    if (empty($description)) {
        $errors['register_errors']['description'] = 'Description is required';
    }
    if (empty($cellphone)) {
    ...
    if (empty($errors)) {
        $sql = "UPDATE `users` SET `company_name`=?, `description`=?, `cellphone`=?, `bank_chard`=? ,`password`=?, `avatar`=? WHERE `id`=?";
        $stmt = mysqli_prepare($connect, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ssssssi", $company_name, $description, $cellphone, $bank_chard , $hashedPassword ,$targetFilePath ,$user_id);
